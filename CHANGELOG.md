...
- **`transports.js` was served stale to browsers holding a cached copy.**
  `app.js` and `machines.js` imported it as `./transports.js?v=8330273` while
  its content hashed to `6434176`, so the editable `remote_path` field added in
  `0a80437` would not have appeared for an existing user until a hard refresh.
  Cache-busting query strings are generated by `bin/wc-asset-versions.py`; that
  commit edited the file without re-running it.
- **`POST /api/sessions/{id}/resume` explains its 404 in the log again.** The
  transcript fallback added on 2026-09-13 rewrote the error message and dropped
  the log line that named the session id and said it was absent from both
  `~/.claude/sessions` and the transcript directory. The request was still
  logged by the central handler, just no longer with that detail.

### Testing

Six defects in the test suite itself, all found by running `rules.md` end to
end. None changed what the product does; together they were hiding whether it
worked.

- **The whole of `tests/test_qa_turn_finished.py` had been dead for four days.**
  Adding a refresh button to the composer toolbar on 2026-09-10 gave
  `createConversationController` a new element to wire, and the file's
  hand-listed stub fixture did not have it, so all ten cases threw before
  reaching an assertion. The fixture now stubs on demand.
- **A duplicated test name meant one test had never run.** Two cases in
  `tests/test_qa_standby_routes.py` shared a name, so Python bound it to the
  second and the standby case was silently discarded. Surfaced by filtering
  flake8 to undefined-name and redefinition classes instead of reading the
  first twenty lines, which were all line-length noise.
- **Three source-text tests were reporting defects that did not exist.** Two
  took a fixed character window after a marker string and the code had grown
  past it; one searched for a single `@media(max-width:620px)` block in a
  stylesheet that now has two, and matched the wrong one. Extraction is now
  bounded by the block rather than by a character count.
- **A running-dot test raced the render.** It waited for a row to exist and then
  asserted on decoration applied a tick later — measured pass, fail, pass over
  three consecutive runs. It now waits for the dots.
- **Nine tests asserted contracts that were deliberately changed on
  2026-09-11.** Transports became a shared pool across accounts, and usage and
  statistics reporting became account-wide. The tests still asserted per-owner
  isolation. They have been inverted rather than deleted, so restoring scoping
  would fail a test rather than pass quietly — and the consequence is now
  written down where a reader will meet it: any logged-in account can see the
  whole host's spend, including another account's conversation titles on the
  usage charts. That is the accepted design for a single-operator deployment.
- **Several tests seeded owner columns with a login name instead of a user id.**
  The owner column has held a UUID since login switched from `user["name"]` to
  `user["id"]`, so those rows belonged to nobody and the owner-scoped reads
  found nothing.
