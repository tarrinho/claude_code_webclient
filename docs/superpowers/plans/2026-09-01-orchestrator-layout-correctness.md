# Orchestrator Layout Correctness Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the orchestrator page's Task Detail panel reachable and give the Event Log the full-width bottom bar its CSS already assumes.

**Architecture:** `web/orchestrator.html` carries a stray `</div>` that closes `#layout` before `#panel-right` opens, ejecting the Task Detail panel to become a sibling of `<body>` where it renders off-screen and cannot be scrolled to. The fix removes that closer, wraps the layout row and the bottom bar in a new `#shell` flex column, and relocates `#panel-bottom` out of the horizontal row. Two small JS changes follow: clear the event log when switching supervisors, and move drag listeners from the 5px handle to `document`.

**Tech Stack:** Vanilla JS (no framework, no build step), inline `<style>` in `orchestrator.html`, FastAPI static serving, pytest + headless chromium via `subprocess` for UI assertions.

**Spec:** `docs/superpowers/specs/2026-09-01-orchestrator-layout-correctness-design.md`

## Global Constraints

- **Use targeted string-replacement edits only on `web/orchestrator.html` and `web/orchestrator.js`. Never a whole-file write.** The 0.9.5 version strings at `orchestrator.html:6`, `orchestrator.html:947` and `orchestrator.js:1452` are committed content at HEAD (`ede43a1`), not anyone's in-flight work — but a whole-file write from a stale read would still revert them and break `tests/test_qa_version_consistency.py`, and this tree acquires an in-flight bump every few hours. Hygiene applies regardless of who is mid-release.
- **Do not edit the version strings.** They are not in scope. If a version string changes, the edit was wrong — revert it. Check with `git diff -- <file> | grep -c '^[+-].*0\.9\.'`, anchored to `+`/`-`: an unanchored `grep -c '0\.9\.'` also counts *context* lines, so it reports a match whenever an edit lands within three lines of `orchestrator.html:947` — a false alarm that invites "fixing" something that never changed. The authoritative check is `diff <(git show HEAD:web/orchestrator.html | grep '0\.9\.') <(grep '0\.9\.' web/orchestrator.html)`, which compares content and ignores line movement.
- **Coordination:** cleared. cweb5 confirmed they hold nothing uncommitted in either file, and `git status --porcelain web/orchestrator.html web/orchestrator.js` was empty at plan time, with `git diff HEAD --stat` on both files empty too. Re-check both before the first edit anyway — three readings of this tree went stale mid-session, in both directions.
- **Known pre-existing failure, not caused by this plan:** `tests/test_qa_supervisor_ux_shortcuts.py` fails 27 of 28 with 180s timeouts. It is cweb5's harness, which drives the real page through raw `chromium --headless --dump-dom` and depends on Chromium exiting by itself; Chromium now hangs when `orchestrator.js` is loaded. cweb5 verified the page is byte-identical to when those tests passed and is migrating the harness to the repo's playwright fixture. **Do not attribute those 27 failures to this change, and do not try to fix them here.** Record the count before the first edit so the comparison is available.
...
- Produces: `probe_geometry(width:int=1440, height:int=900, body_class:str="") -> dict[str,str]` — loads the real `orchestrator.html` with its `<script>` tag stripped, returns a dict parsed from `document.title`, keyed by selector, values `"x,y WxH"`, plus `"panel-right.parent"` and `"viewport"`. Tasks 3 and 7 reuse it.
...
```python
"""The orchestrator page's four panels must all be reachable.

`web/orchestrator.html` shipped a stray `</div>` that closed `#layout` before
...
SUPERVISOR_HTML = REPO / "web" / "orchestrator.html"
...
def body_markup() -> str:
    """The <body> of orchestrator.html with HTML comments stripped.
...
def probe_geometry(width: int = 1440, height: int = 900,
    ...
    Chromium does not exit when orchestrator.js is loaded under --dump-dom -- it
    ...
    failing 27 of 28 at the time of writing. Measured by cweb5:
        ...
        orchestrator.html alone, no orchestrator.js    rc=0,   1s
        orchestrator.html + orchestrator.js, no stubs  rc=124, hangs
    ...
    """
    html = SUPERVISOR_HTML.read_text(encoding="utf-8")
    html = re.sub(r'<script src="orchestrator\.js[^"]*"></script>', "", html)
    if body_class:
...
class MarkupNestingTests(unittest.TestCase):
    ...
    def test_the_body_divs_balance(self):
        ...
        self.assertEqual(depth, 0,
                         "unbalanced <div> nesting in orchestrator.html body")
...
git commit -m "test: assert orchestrator panels are reachable (currently failing)" -- tests/test_qa_supervisor_layout.py
```
...
**Files:**
- Modify: `web/orchestrator.html:1029` (delete one line)
...
Current markup, `web/orchestrator.html:1025-1032`:
...
Run: `git diff -- web/orchestrator.html | grep -c '^[+-].*0\.9\.'`
...
```bash
git add web/orchestrator.html
git diff --cached --stat   # must list ONLY web/orchestrator.html
...
could see." -- web/orchestrator.html
```
...
**Files:**
- Modify: `web/orchestrator.html` — markup 950 (`#layout` open), 1019-1028 (`#panel-bottom` block moves), 1042 (`#layout` close); CSS 46-51
- Test: `tests/test_qa_supervisor_layout.py`
...
```
    ...
    <script src="orchestrator.js?v=4"></script>
```
Replace with:
```
        </div>
...
```bash
git add web/orchestrator.html tests/test_qa_supervisor_layout.py
...
despite border-top and a max-bottom rule using left:0/right:0." -- web/orchestrator.html tests/test_qa_supervisor_layout.py
```
...
**Files:**
- Modify: `web/orchestrator.html` — add one handle inside `#panel-bottom`; add `.resize-handle.horizontal` after the CSS at 101-114
...
Run:
```bash
grep -c 'data-resize="bottom"' web/orchestrator.html   # expect 1
git diff -- web/orchestrator.html | grep -c '^[+-].*0\.9\.'   # expect 0
```
...
```bash
git add web/orchestrator.html
git diff --cached --stat   # ONLY web/orchestrator.html
...
path was unreachable." -- web/orchestrator.html
```
...
**Files:**
- Modify: `web/orchestrator.js` — `reinitResizeHandles()` (~1142) and `onResizeEnd()` (~1134)
...
Run: `git diff -- web/orchestrator.js | grep -c '^[+-].*0\.9\.'`

Expected: `0`. `orchestrator.js:1452` is cweb5's `"0.9.5"`.

- [ ] **Step 4: Check the file still parses**

Run: `node --check web/orchestrator.js && echo "parses"`
...
```bash
git add web/orchestrator.js
git diff --cached --stat   # ONLY web/orchestrator.js
...
mid-gesture. Moved to document and released in onResizeEnd." -- web/orchestrator.js
```
...
`addLogEntry()` only appends to the DOM, and `selectSupervisor()` never clears it, so switching supervisors interleaves two runs' events with no divider. `chatMessages` is already replaced per orchestrator; this brings the log into line.

**Files:**
- Modify: `web/orchestrator.js` — `selectSupervisor()` (~394)
- Test: `tests/test_qa_supervisor_layout.py`
...
class EventLogResetTests(unittest.TestCase):
    """One orchestrator's events must not appear under another's name.
    ...
    def setUp(self):
        self.source = (REPO / "web" / "orchestrator.js").read_text(
            encoding="utf-8")
    ...
    def test_it_is_cleared_before_the_panel_repopulates(self):
        ...
        self.assertLess(
            ...
            "the log must be cleared before the new orchestrator is shown",
        )
```
...
Expected: `test_it_is_cleared_before_the_panel_repopulates` FAILS. Confirm the file actually changed (`git diff -- web/orchestrator.js | head`) before drawing a conclusion — a mutation that did not land reports a false pass. Then restore the correct order and re-run to green.

- [ ] **Step 6: Commit**

```bash
git add web/orchestrator.js tests/test_qa_supervisor_layout.py
...
events interleaved with no divider." -- web/orchestrator.js tests/test_qa_supervisor_layout.py
```
...
The `body.max-*` rules are the trap in this change: `#shell` acquiring a containing block would break all four while the page still looks correct at rest. This task asserts that explicitly, then bumps the script version so a cached `orchestrator.js` cannot mask Tasks 5 and 6.

**Files:**
- Modify: `web/orchestrator.html:1044` (script version)
- Test: `tests/test_qa_supervisor_layout.py`
...
Replace with:
```
    <script src="orchestrator.js?v=5"></script>
```
...
Run:
```bash
git diff -- web/orchestrator.html | grep '^[+-].*orchestrator\.js?v='   # the bump
git diff -- web/orchestrator.html | grep -c '^[+-].*0\.9\.'                  # expect 0
...
After all four, confirm the tree is restored: `git diff -- web/orchestrator.html` should show only this task's script-version bump.
...
A green suite does not show a person a panel. Load the orchestrator page and confirm:
...
git add web/orchestrator.html tests/test_qa_supervisor_layout.py
git diff --cached --stat   # ONLY those two
git commit -m "test: assert maximize survives the #shell wrapper; cache-bust orchestrator.js
...
looked correct at rest." -- web/orchestrator.html tests/test_qa_supervisor_layout.py
```
...
- **`git status --porcelain web/orchestrator.html web/orchestrator.js` before each task.** This tree has moved under a measurement roughly every ten minutes; a clean reading goes stale within minutes. If either file has changed under you, re-read before editing.
