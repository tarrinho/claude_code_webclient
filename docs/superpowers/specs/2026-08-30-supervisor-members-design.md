# Supervisor members — supervising existing chats and agents
...
A supervisor cannot be pointed at work that already exists. It decomposes a
...
there is no link anywhere between a supervisor and a conversation or a running
...
2. **A human clicks send.** The supervisor never dispatches on its own. See
   *Why the human stays in the loop* below; this is a safety property, not a
   convenience.
3. **Two entry points.** A bulk picker inside the supervisor, and
   `⋯ → Add to supervisor ▸` on a conversation row.
...
supervisor scheduler dispatching unattended would remove that check, at machine
speed, on prompts chosen by a model's plan. So the supervisor prepares a prompt
...
Returns the same entry shape `/api/supervisor` already produces, filtered to
this supervisor's members:
...
Removes membership only. It never deletes the conversation — a supervisor is a
...
crafted `ref_id` would pull another account's conversation into a supervisor
...
**Picker (bulk).** `[+ Add members]` on the supervisor page opens a filterable
...
**Row menu (one-off).** `⋯ → Add to supervisor ▸` in `chat-list.js`, listing the
...
and the controls. Polls the members endpoint on the interval the supervisor
page already uses.
...
- **A member's chat is deleted.** The join returns nothing for it. The row is
  ...
  supervisor listing a conversation that no longer exists is the failure worth
  preventing.
...
- **The members feed fails.** The panel renders the last good list with a stale
  marker. The supervisor page must not go blank because one poll failed —
  matching how the sidebar treats `/api/supervisor`.
...
- **Security:** another owner's `chat_id` is refused; another owner's session id
  is refused; a member cannot be added to a supervisor the caller does not own.
- **Adoption:** adding a session stores a `chat_id`, not a session id; adding
  the same agent twice yields one member and one chat.
- **Status reuse:** a member's status matches what `/api/supervisor` reports for
