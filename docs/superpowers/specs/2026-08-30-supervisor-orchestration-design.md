...
Extend WebConsole with a supervisor agent that orchestrates multiple Claude Code turns as a structured plan with subtasks, dependencies, model assignment, and real-time progress tracking. The supervisor is itself a Claude Code session that reasons about the user's request, breaks it into subtasks, assigns the best model for each, and manages execution — all streamed to the user through an integrated three-panel UI.
...
- Let the supervisor choose models per subtask based on complexity and cost (simple questions → free/fast models, complex work → capable models).
...
- Let the user interact with the supervisor at any time to adjust the plan, answer agent questions, or add subtasks.
...
- Multi-user supervisor sessions (single user per supervisor).
- Persistent supervisor plans across sessions (plans live for the duration of the session).
- Visual-only deployment changes beyond adding a new route and frontend files.

## Architecture

Add a supervisor module that sits alongside the existing runner. The supervisor is itself a Claude Code session (a "supervisor chat") that receives the user's request, reasons, and produces a structured plan. The frontend consumes this plan and drives the multi-panel layout.

```
Browser (vanilla JS SPA)
  ├── Existing chat UI (index.html + app.js + chat-list.js + conversation.js)
  └── Supervisor UI (supervisor.html + supervisor.js + supervisor.css)  [NEW]

FastAPI (app.py — extended)
  ├── /api/supervisors          [NEW] — CRUD supervisor orchestrations
  ├── /api/supervisors/{id}/stream [NEW] — SSE for supervisor reasoning + plan
  ├── /api/supervisors/{id}/tasks [NEW] — Subtask CRUD + status
  ├── /api/supervisors/{id}/tasks/{taskId}/stream [NEW] — SSE per subtask
  └── Existing endpoints (unchanged)

supervisor.py                 [NEW] — Orchestration logic
  ├── PlanParser — Extract structured plan from supervisor's reasoning
...
runner.py (extended)
  ├── run_supervisor_turn()    [NEW] — Run a single turn under supervisor context
  └── Existing turn flow (unchanged)

transcripts.py (extended)
  ├── supervisor transcript export [NEW]
...
1. User sends a request to the supervisor: `"Deploy the new API changes across staging and production"`
2. Supervisor creates a new supervisor session (a chat with `supervisor=1`).
3. The supervisor session runs as a streaming turn — the supervisor (a Claude Code instance) reasons about the request, asks clarifying questions if needed, and produces a structured plan.
4. The plan is parsed from the supervisor's response into structured data:
   - Subtasks with titles, descriptions, and instructions
   - Dependencies between subtasks
   - Suggested models per subtask
   - Estimated complexity (simple / moderate / complex)
5. The plan is streamed back to the frontend in real-time as the supervisor thinks.
...
7. The frontend renders the three-panel layout:
   ...
   - **Bottom panel**: Supervisor chat (user ↔ supervisor conversation)
   - **Right panel** (toggleable): Detail view for the selected subtask
8. The supervisor immediately begins executing subtasks that have no blockers.
...
10. As each subtask runs, the supervisor monitors progress and updates the task status.
11. When a subtask completes, dependent subtasks become eligible for execution.
12. The supervisor tracks overall progress and reports it to the frontend.
13. The user can interact at any time:
    - Click a different subtask to see its conversation
    - Message the supervisor to adjust the plan
    - Ask a question (supervisor can answer from its own reasoning or delegate to an agent)
    - Add new subtasks
    - Stop all execution
14. When all subtasks complete, the supervisor presents a summary.

### Supervisor Intelligence

The supervisor uses a system prompt template that instructs it to:
...
4. When agents ask questions, the supervisor either:
...
The supervisor's output format is a hybrid: prose reasoning + structured JSON markers that the frontend parser can extract.
...
CREATE TABLE supervisors (
    ...
    chat_id       TEXT NOT NULL REFERENCES chats(id),  -- links to the supervisor chat
...
- `chats.supervisor` column (nullable): links a chat to its parent supervisor. When a task creates a chat, this is set.
  ```sql
  ALTER TABLE chats ADD COLUMN supervisor TEXT REFERENCES supervisors(id);
  ```
...
POST /api/supervisors
{
  ...
  "model": "claude-sonnet-4-20250514"  // optional: which model to use as the supervisor itself
}
→ {
  "id": "uuid",
  "chat_id": "uuid",  // the supervisor chat id
  "title": "Deploy...",
  "status": "planning",
  "progress": 0,
...
### Stream Supervisor Planning

```
POST /api/supervisors/{id}/stream
→ SSE stream of the supervisor's reasoning + structured plan extraction
...
The supervisor processes the message and streams its response. The frontend can pipe this through the SSE endpoint or POST /stream returns the SSE URL directly.
...
→ {
  "supervisor": { ... metadata ... },
  "tasks": [ ... task list ... ],
  "messages": [ ... supervisor chat messages ... ]
}
```
...
Cancels all running tasks and removes the supervisor record.
...
Extracts structured plans from the supervisor's free-text reasoning. Uses regex + JSON block detection:

```python
class PlanParser:
    @staticmethod
    def extract_plan(text: str) -> dict | None:
        """Extract structured plan from supervisor's response text.
...
class SupervisorEngine:
    ...
    async def plan(self, user_request: str, model: str) -> str:
        """Run the supervisor turn to generate the plan.
    ...
    async def handle_user_message(self, supervisor_id: str, content: str) -> AsyncGenerator:
        """Process a user message to the supervisor.
        Streams the supervisor's response as SSE events.
        """
...
The supervisor respects the existing concurrency model:

- The supervisor itself uses one concurrency slot (the `MAX_CONCURRENT` semaphore).
- Each subtask agent also uses one concurrency slot.
- To prevent exhaustion, the supervisor limits parallel subtask execution to `min(MAX_CONCURRENT - 1, 3)`, leaving at least one slot available for other operations.
- Tasks that depend on each other execute sequentially regardless.

## SSE Event Protocol

### Supervisor Planning Stream
...
├── supervisor.html                   # NEW: Supervisor UI shell
├── supervisor.css                    # NEW: Supervisor layout styles
├── supervisor.js                     # NEW: Supervisor orchestration logic
├── assets/
│   ├── supervisor-mockup.html        # Existing: design mockup
│   └── supervisor-task-tree.js       # NEW: Task tree component
└── ...                               # Existing files (unchanged)
```

### Routes

Add `/supervisor` and `/supervisor/{path}` to the existing `app.py` static file serving, served as `supervisor.html` (SPA pattern).
...
│      │                              │              │
├──────┴──────────────────────────────┴──────────────┤
│  Supervisor Chat (bottom panel, resizable)         │
├────────────────────────────────────────────────────┤
...
The supervisor UI maintains a single state object:

```javascript
const state = {
  supervisorId: null,
  activeChatId: null,       // which chat/task is currently shown
  supervisor: null,         // supervisor metadata
  tasks: new Map(),         // task_id → task data
  messages: [],             // supervisor messages (recent)
  streams: new Map(),       // task_id → AbortController
  status: 'planning',       // current supervisor status
...
### Supervisor Chat (Bottom)

The bottom panel is the direct conversation with the supervisor:

- User messages and supervisor responses
...
The supervisor is instructed via a system prompt template:

```
You are a deployment supervisor orchestrating a multi-step plan.
...
The supervisor's SSE endpoints follow the same pattern as the existing stream endpoint:
...
Supervisor endpoints require authentication (same auth middleware). The user must own the supervisor (linked to their chat). All supervisor operations are single-user (no multi-user isolation needed).

### Concurrency Integration

The supervisor shares the existing concurrency semaphore with regular chats:
- Supervisor planning turn: 1 slot
...
- Supervisor engine: full lifecycle (plan → tasks → execute → complete), pause/resume, cancel, failure recovery
...
- Auth: supervisor isolation by owner_id
...
- Supervisor chat: send message, receive response, plan display
- Error states: failed tasks, network errors, supervisor failure
- Keyboard navigation: tab, arrow keys, enter to select
- Responsive layout: works at various viewport widths

### Acceptance

1. Create a supervisor with a multi-step request.
...
5. Send a message to the supervisor to add a subtask.
6. Verify progress percentages update in real-time.
7. Pause and resume a running supervisor.
8. Delete a supervisor and verify tasks are cancelled.
9. Export the supervisor transcript as Markdown.
10. Verify the mockup matches the actual implementation.

## UI Design Notes

The mockup at `web/assets/supervisor-mockup.html` covers the visual design (served at `https://kali-2.tail850c40.ts.net/assets/supervisor-mockup.html`). The implementation should match this mockup with:
...
- Three-panel layout: task tree (left), conversation (center), supervisor chat (bottom)
- Status bar between center and bottom panels

## Deployment

Add `/supervisor` route in `app.py` to serve `supervisor.html`:

```python
app.add_route("/supervisor", handle_supervisor_page, methods=["GET"])
app.add_route("/supervisor/{path:path}", handle_supervisor_page, methods=["GET"])
```

New static files served via existing `StaticFiles` mount at `/assets/`:
- `supervisor.css` → `/assets/supervisor.css`
- `supervisor.js` → `/assets/supervisor.js`
- `supervisor-task-tree.js` → `/assets/supervisor-task-tree.js`

## Implementation Phases

### Phase 1: Data Model & API
- Add supervisor tables to `db.py` with migration
- CRUD endpoints for supervisors, tasks, and messages
- PATCH for status transitions
- GET for listing and detail views

### Phase 2: Supervisor Engine
...
### Phase 4: Frontend
- `supervisor.html` shell with three-panel layout
- `supervisor.css` styling matching the mockup
- `supervisor.js` state management and API integration
- `supervisor-task-tree.js` tree component
