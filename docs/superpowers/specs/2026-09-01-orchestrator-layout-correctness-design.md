# Orchestrator layout correctness — making the panels reachable

The orchestrator page has four panels. One of them has never been on screen, and
...
The cost is a whole feature. `renderTaskDetail()` (`web/orchestrator.js:664`)
...
`2026-09-01-orchestrator-ux-design.md`, which lists "results hard to find" as
...
(`web/orchestrator.js:1121`) and `reinitResizeHandles` already special-cases
...
### 3. JS — `web/orchestrator.js`

- `selectSupervisor()` empties `#event-log` and sets `eventLog.length = 0`
  before `showActiveSupervisor()`, so the log always describes the orchestrator on
  screen. This mirrors how `chatMessages` is already replaced per orchestrator.
...
- **All colour and token work.** `orchestrator.html` links no stylesheet and
  ...
  **The light palette here is deliberate**, not an oversight:
  `2026-08-30-orchestrator-orchestration-design.md` §*UI Design Notes* specifies a
  "clean blue light palette" matching `web/assets/orchestrator-mockup.html`. So
  ...
  `2026-09-01-orchestrator-ux-design.md`'s "both themes" verification can pass.
...
- **Live task output and Retry.** Owned by
  `2026-09-01-orchestrator-ux-design.md`. That design's §3 feeds a live section
...
`2026-08-30-orchestrator-orchestration-design.md` §*Frontend Architecture*, which
is the foundational design. Recorded here so a later reader can tell intent from
drift:

| Orchestration spec | As built | Status |
|---|---|---|
| Bottom panel = **Orchestrator Chat** | Bottom = **Event Log** | Divergence, accepted; the Event Log is not in that spec at all |
| Centre panel = the selected **subtask's conversation** (its chat + SSE) | Centre = **Orchestrator Chat** | Divergence; the per-subtask conversation view **does not exist** — a real gap, unclaimed by any spec |
...
| Separate `orchestrator.css` + `orchestrator-task-tree.js` under `/assets/` | One 930-line inline `<style>`; `orchestrator.js` served from `/orchestrator.js` | Divergence; the extraction is a precondition for **D** |
...
the orchestration spec puts Orchestrator Chat in that slot, this design puts the
...
| File | Change |
|---|---|
| `web/orchestrator.html` | `#shell` wrapper; `#panel-right` moved into `#layout`; stray `</div>` removed; bottom resize handle; two CSS rules |
| `web/orchestrator.js` | log cleared in `selectSupervisor()`; drag listeners moved to `document` |
| `tests/test_qa_supervisor_layout.py` | new — geometry and nesting assertions |

The script tag needs its cache-buster bumped (`orchestrator.js?v=4` → `v=5`), and
...
5. **Log clearing.** Selecting a different orchestrator leaves `#event-log`
...
`web/orchestrator.html` and `web/orchestrator.js` are both dirty with peer work, and
