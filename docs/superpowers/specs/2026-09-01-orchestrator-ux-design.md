# Orchestrator UX — live task output, and a way back from a failure
...
Four UX complaints were raised against the orchestrator page: task progress is
...
Both turns the orchestrator makes go through the blocking entry point:

```python
chunks, _sid = await runner.run_turn(...)      # orchestrator.py:589 (planner)
chunks, _sid = await runner.run_turn(...)      # orchestrator.py:802 (each task)
result = "".join(chunks) if chunks else ""
```

`run_turn` returns only when the turn is over. `runner.stream_turn` exists
beside it and yields events as they arrive, but the orchestrator never calls it.
...
There is no retry anywhere in the orchestrator: no endpoint, no button, no engine
method. Grep for it finds only rate-limit `Retry-After` headers and the
conversation-level retry in `app.py`. When a run fails the orchestrator sits at
...
`orchestrator.py` and `web/orchestrator.js` are **dirty with a live peer's
...
The orchestrator must pass `owner`. Its chat ids (`subtask_<id>`,
...
argument at `orchestrator.py:585`, and the comment there says so.
...
orchestrator change, so a regression here cannot be confused with a bug in the
streaming work.
...
chat path. The orchestrator never calls it on either path, so orchestrator token
...
Both are in `web/orchestrator.js`, which is dirty. **Coordinate before editing**
(see *Coordination*).
...
- Refuses unless `status` is `error` — retrying a `running` orchestrator would
  start a second engine against one graph. `409` with a reason, not a silent
  no-op.
- Re-sends the **last `user` message** for that orchestrator, read back from
  `supervisor_messages`. No new prompt is invented and none is required from the
  UI.
- Clears the previous run's tasks for that orchestrator before replanning.
...
In the UI: an error-state orchestrator row and the failure message both get a
...
| `orchestrator.py` | `_execute_task` streams; `_emit_task_output`; `retry()` |
| `app.py` | `POST /api/supervisors/{id}/retry` |
| `web/orchestrator.js` | `task_output` in the event log and detail panel; Retry button |
...
3. **Mutation-check both new suites before claiming them,** and verify each
   ...
   `retry` accept a `running` orchestrator; return the wrong `user` message.
...
5. Kill the proxy mid-task and confirm the failure reaches the chat with its
   reason, the orchestrator lands on `error`, and Retry appears and works.
6. Retry a orchestrator whose failure cannot be retried away (stop the proxy) and
...
`orchestrator.py` and `web/orchestrator.js` are dirty with a peer's uncommitted work
right now, and `web/orchestrator.js` is where two of these four changes land.
...
`renderTaskDetail`, and the orchestrator-row template — and wait for
