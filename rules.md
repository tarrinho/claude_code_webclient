...
| 109 | A delete-confirmation dialog opened from Settings > Images rendered *behind* the Settings panel itself -- reported by Pedro: "the delete popup appeares below the popup of the page it self" | `#confirmDialog` (the delete confirm shared by images.js/machines.js/specs.js) and `#settingsDialog` are both `.dialog-backdrop`, both at the base `z-index:400`. Equal z-index falls back to DOM order, and `#confirmDialog` sits earlier in `index.html` than `#settingsDialog`, so the later-painted Settings backdrop covered it whenever both were open. `#specViewerDialog` hit the identical class of bug earlier (its own CSS comment already explains why) and was bumped to `401` for exactly this "opened from within Settings" reason -- `#confirmDialog` never got the same fix, despite being the more commonly hit path (every Settings-tab delete, not just one viewer) | `#confirmDialog` added alongside `#specViewerDialog` at `z-index:401` | `tests/test_qa_confirm_dialog_stacking.py`: renders the real `index.html`+`styles.css` headlessly (same technique as `test_qa_auto_answer_geometry.py`), forces both dialogs `.open`, and asks the browser itself which element `elementFromPoint` resolves to at the confirm dialog's own center -- the same question a real click answers. Mutation-verified: reverting the CSS reproduces the exact symptom, `elementFromPoint` resolves to an element inside `#settingsDialog` (`id="backendMap"`), not `#confirmDialog`. A source-only z-index-number check would not have caught this: the two dialogs' *values* were already different-looking (400 vs whatever a naive read expects) only after this fix -- before it, both were legitimately 400, so the bug was in the stacking outcome of equal values plus DOM order, not in a wrong number visible from either file alone |
| 110 | `GET /api/images` and every `routes/transports.py` endpoint (e.g. `GET /api/transports/sync-requests/pending`) returned 500 in production. Found while auditing an unrelated spec (the images gallery), not reported first | The systemic `session["user"]` -> `owner_of(session)` refactor (`6de2bd4` onward) rewrote the ownership line in both files' handlers, but neither file gained `from shared import owner_of` -- every request raised `NameError: name 'owner_of' is not defined`, confirmed live via direct request against the running service before touching anything | Added the missing import to both files (`3788115`) | `tests/test_qa_images_api.py` and `tests/test_qa_transports.py` call the real handlers with no mock of `owner_of`. Mutation-verified: removing the import from both files again failed 34 of 43 cases across the two files with the exact `NameError`; restoring it returned 43/43 passed |
| 111 | `web/assets/transports.js` was served stale to any browser holding a cached copy: `app.js` and `machines.js` both imported it as `./transports.js?v=8330273` while its content hashed to `6434176`. The `remote_path` field added in `0a80437` would not have appeared for an existing user until a hard refresh. | `0a80437` edited `transports.js` without re-running `bin/wc-asset-versions.py`, so the cache-busting query string kept pointing at the previous content. | Ran `bin/wc-asset-versions.py`; it rewrote both import sites to `?v=6434176`. | `tests/test_qa_asset_versions_match_content.py` (pre-existing, and it is what caught this -- three of its five cases were failing). It is a detector, not a preventer: nothing stops the next edit from shipping stale until the suite runs. |
| 112 | Every test in `tests/test_qa_turn_finished.py` -- ten cases covering whether a turn reports that it finished -- had been failing since 2026-09-10, four days. `Page.evaluate: TypeError: Cannot read properties of undefined (reading 'addEventListener')` at `conversation.js:1495`. | `850880f` added a refresh button to the composer toolbar, so `createConversationController` began calling `elements.refreshChatBtn.addEventListener`. The test's fixture is a hand-listed set of stub elements and did not gain the new name, so the controller threw before any assertion ran. The whole file failing at once is what made it look like infrastructure rather than a stale list. | The fixture's element object is now a `Proxy` that stubs any property the controller reaches for. The named entries stay, because the assertions read those back. The trade-off is written into the test: an element missing from `index.html` would now be stubbed rather than caught here, and `tests/test_qa_queue_button.py` is what covers that. | The ten cases in `tests/test_qa_turn_finished.py` themselves -- they failed before the change and pass after it. |
| 113 | `tests/test_qa_standby_routes.py` defined `test_400_when_chat_has_no_session_id` twice. Python bound the name to the second definition, so the first -- the standby case -- had never run since it was written. | Two tests written for two different routes (standby and wake) were given the same name. Nothing reports this: the file passed, with one fewer test than it appeared to contain. | Renamed the second to `test_wake_400_when_standby_flag_is_stale`. The previously-shadowed test passes; the file went from 9 collected to 10. | Found by flake8 F811, which is already in §9. The stage's `head -20` had been hiding it behind E501 noise -- filtering to `--select=F,E9` is what surfaced it. |
| 114 | `POST /api/sessions/{id}/resume` returning 404 wrote nothing to the log explaining which lookup failed. | `d4719a4` (2026-09-13) added the transcript fallback and rewrote the 404's detail string, dropping the `_log.info` that named the session id and said it was absent from both `~/.claude/sessions` and the transcript directory. The central HTTP error handler still logs the status and detail, so the route was not silent -- just no longer specific. | Restored the log line in `routes/misc.py`'s 404 branch. | `tests/test_qa_errors.py::test_session_not_found_logs_guidance` (pre-existing). It had been asserting a marker string absent from the entire tree since d4719a4, which is how the loss was found. |
| 115 | `tests/test_qa_chat_running_dot.py::test_running_agents_each_get_their_own_dot` failed non-deterministically -- measured pass, fail, pass over three consecutive runs with no code change. | The test waited for the `c3` row to exist and then asserted immediately. The rows render first and the running dots are applied on a later tick, so the assertion sometimes read the list mid-decoration. The indicator itself is correct; a dot arriving one tick after its row is imperceptible. | Added `_wait_for_dots()`, which waits for the dots themselves rather than for a row. The negative assertion (c3 has none) stays meaningful because the dots are applied to the whole list in one pass. | The file's own nine cases, run three times in a row after the change, all green. |
| 116 | Three source-text tests reported defects that did not exist: `test_polling_starts_when_a_chat_is_selected`, `test_escape_closes_the_menu_and_restores_focus` (both `tests/test_qa_auto_answer_ui.py`) and `test_the_transport_column_is_hidden_with_the_wires` (`tests/test_qa_backend_map_columns.py`). All three assert on strings that were present in the source the whole time. | Two failure shapes, one cause -- extraction that does not track the file. The first two took a fixed character window after a marker (`split(marker,1)[1][:2000]`); `updateCurrentUi` had grown to 2,634 characters and the Escape handler to 1,691, so the window cut off before the target. The third used `re.search` for `@media(max-width:620px)` when `styles.css` now has two such blocks, and matched the earlier one, which never contained `.map-via`. | `_body_after()` bounds an extraction on the block's closing brace instead of a character count, and asserts that a closer was actually found; the CSS test switched to `re.findall` and joins every matching block. | The three cases themselves, red before and green after. Worth noting what these are: a too-small window invents a failure, a "safely large" one silently searches the next function, and neither is visible from a passing report. |
| 117 | Revision 3 of `docs/superpowers/specs/2026-09-15-worktree-per-session-isolation-design.md` -- 166 insertions closing the last decisions before implementation -- was committed by another session as `e9731c3 "bench: four edit-shaped coding tasks, to measure the workload that exists"`, a commit about benchmark fixtures. `git commit` from the authoring session then answered **"no changes added to commit"** against an index it had just staged | #52 exactly, recurring: a peer's whole-tree `git add` swept a staged file belonging to another session. Nothing was lost and nothing was mangled -- the content in HEAD is byte-identical to what was staged -- but the change is filed under a message that does not mention it, so `git log -- <spec>` attributes the spec's largest revision to benchmark work, and the reasoning written for that commit message never landed anywhere. The timing is the useful part rather than an irony: this happened **while committing the spec whose subject is this failure**, in the window between `git add` and `git commit`, which is the narrowest form of the race and still wide enough | None available at the time. The authoring session verified the content was intact rather than re-committing, since rewriting a pushed peer commit is destructive and was not its call. The rationale survives because it was also written into the document itself (§12's revision-3 table), which is the more durable place for it -- a design's reasons belong in the design, not only in a commit message that a sweep can silently reassign | The spec this happened to is the regression test, once implemented: §4.2 denies the write and §4.2.1 denies the destructive git. Recorded here as evidence rather than as a new problem -- it is the fifth documented instance of the same sweep, and the first with a timestamp inside the fix's own authoring. Note for the migration in that spec's §7: a `git add` race is not closed by worktrees alone until every session is moved, because one session still sweeping a shared index can capture a file another session staged there |
| 118 | Settings > Delegation reported "Could not change the operational flag" for every knob, and "Could not save" for every rejected cell edit -- never the server's actual reason | This app serialises errors as `{"error": ...}` (`app.py`'s `handle_http_exception`); `delegation.js` read `data.detail` in both write paths, so the reason was always `undefined` and the generic fallback always won. FastAPI's `detail` name never reaches the browser. The cell-edit half is the worse one: spec 1.1 requires a refused write to name the broken invariant and the offending column, and the server did -- the page discarded it. Nothing caught it because the route tests assert the API response, which was correct throughout; the loss happened in the browser, where no test was looking | Extracted `_errorMessage(data, fallback)` reading `error` first, `detail` second, falling back on a blank or non-object body | `DelegationErrorMessageTests` in `test_frontend_syntax.py` -- executes the shipped function under QuickJS rather than matching source text. Mutation-verified: reverting to `data.detail` fails exactly the two tests naming the defect |
| 119 | Delegation cards read "multi-turn: **is operational** but its ladder is empty" for task types that are not operational, contradicting the status band directly above them | The string was written for `validate()` running against genuinely operational types, where it is true. The settings page reuses the same validation against a *hypothetical* candidate ("what would break if this were flipped") to show why a type is blocked, and the wording did not follow. Every other problem string in `validate()` describes a condition rather than asserting state; this was the only one that asserted | Reworded to describe the condition without asserting the type's status, so it is true in both callers. Audited every other problem string in `validate`, `_worst_case`, `_tree_cost`, `_resolution_problems`, `_gate_latency_s`, `_gate_climb_latency_s` and `_gate_candidates` for the same shape -- none had it. `orchestrator.py:326` has the same phrasing but is gated by a real `is_operational()` check, so it is correct in context and was left | `test_qa_delegation_spec_coverage.py`'s existing assertion on "ladder is empty" still holds; the reworded string is what it now matches |
| 120 | `bin/wc-seed-delegation.py` still recorded `claude-sonnet-5`'s `reviewer-gate` row as unmeasured after that latency was measured and written into spec 2.6, so a freshly seeded database showed a blank cell and reported the gate-climb model as unmeasured | The test written to prevent exactly this compared `(model, task_type)` pairs -- `{(model, task_type) for model, task_type, *_ in module.ROWS}` -- and `*_` discarded every measured value. It could only ever catch a missing or extra *row*, never a wrong *number*, so the seed could carry stale measurements indefinitely while staying green. It drifted within hours of the values being updated | Updated the row, and rewrote the guard to compare column-by-column against spec 2.6's parsed table, honouring its conventions (`TBD` and em dash as absent, trailing `*` on `n` as a latency-sample marker, thousands separators). All 23 rows compared; no other row had drifted | `SeedRowValuesTests` in `test_qa_delegation_shipped_state.py`. Mutation-verified: changing one seeded number fails with `claude-sonnet-5 / reviewer-gate: median_latency_s is 9.999 in ROWS but spec 2.6 has 3.675`, naming row and column |
| 121 | The read-only config block at the foot of Settings > Delegation rendered unreadable text -- "Cinnat implemented in this release", "Breatext names 60% of the last 20 leaves" | Its `<dl>` used `grid-template-columns: minmax(0,1fr) auto`. Several values are full sentences, so the `auto` value column sized itself to fit them and squeezed the label column toward zero; labels wrapped one word per line, and because `dt`/`dd` share a grid row, the tall label fragments interleaved with the value's own wrapped lines. No test could see it -- which is how it shipped, and why it was found only by screenshotting the page and looking at the image | Stacked label above value (flex column) so a long sentence wraps under its own label, and widened the outer grid track from 220px to 260px, giving three comfortable columns rather than four cramped ones | Playwright geometry assertion in `test_frontend_browser.py` -- added under rules.md 16a, which requires a real browser render for anything a browser lays out |
| 122 | `README.md`'s architecture module map listed `routes/supervisors.py`, which is not a file | The real module is `routes/supervisor_map.py`. The same stale entry existed in `ARCHITECTURE.md` and was removed from there earlier the same day; nobody checked whether the map appeared in a second place. A file inventory listing a file that does not exist is worse than an incomplete one, because a reader trusts it | Corrected the name and added the delegation modules the map omitted, with an explicit note that none of them routes | N/A -- documentation. The `ARCHITECTURE.md` half is covered by `test_qa_architecture_diagrams.py`; README's map has no equivalent gate, which is why this survived there |
| 123 | rules.md 17's case 3 (wedged server) reported a false failure: the health check printed nothing, restarted nothing, and the server stayed down | Case 1 restarts the server moments earlier, and `boot_grace()` in `wc-health.sh` skips the entire check while `MainPID` is younger than `WC_HEALTH_BOOT_GRACE` (45s) -- deliberate, since a process too young to have served a request cannot be wedged. Run back-to-back as the stage was written, case 3 always lands inside that window. The skip is silent, which is what makes it read as a product failure rather than a sequencing one | Added an explicit wait for the grace window to case 3 in stage 17, with the reason, so the next run does not spend the same half hour concluding the health check is broken | N/A -- procedure. Verified by re-running case 3 at 50s uptime: three attempts at HTTP 000, then `unhealthy after 3 attempts; restarting webconsole.service`, and a 200 |
| 124 | Four prospective validation sites built the capability table WITHOUT pins, while `validate_or_die` builds it with pins and then pin-safes them | Both enforcement endpoints, the row-write check and the settings page's blockers used `CapabilityTable(rows, operational=...)`. Those checks exist only to predict what boot will say at the next restart, so a differently-shaped table does not predict it -- and `handle_row_put`'s own comment had asserted the opposite for as long as it was false. Measured on production data 2026-09-26: `multi-turn`'s generated ladder is 3,850s and its pinned ladder 2,675s against a 3,400s ceiling, so the ceiling knob refused a flip boot would have accepted and named a ladder the router does not use. `boot_shaped_table()` is now the single builder and `validate_or_die` uses it too. |

## 16a-audit (this run): §16a ran against entry #124 and the `### Fixed`/`### Changed` lines added this run. #124 is covered by `tests/test_qa_delegation_pin_agreement.py`, mutation-verified: reverting the ceiling endpoint to its pinless table fails `test_the_ceiling_knob_endpoint_honours_the_pin` and nothing else -- which is also why that endpoint case exists at all, since the six builder-level cases in the same file all stayed green under that revert and would have read as coverage they do not provide. Four other mutation checks were run this session and each failed exactly the test written for it: removing `sys_reclaim`'s in-use guard, making `_family` return a family for every name, deleting `voice-interrupt.js`'s echo filter, and dropping `justify-self` from `.srv-action-btn`. The `LATENCY_CEILING_S` and `BUDGET_USD` moves are operator decisions rather than fixes, and are covered the way the budget's history already was -- by `test_opus_is_refused_at_the_shallow_rungs_but_not_the_deepest`, whose ledger was extended to 4.50 rather than rescaled, per its own design note. The cost_basis writes and the two repins changed data, not code, and carry no test by design. Nothing skipped.

## 16a-audit (run before): §16a ran against entries #93-99. #93 and #94 share one new gate (`test_qa_no_undefined_names.py`), mutation-verified by restoring each bug separately; both are undefined names, which is why the gate checks the class rather than the two sites. #95 has `test_qa_d3_stub_fidelity.py`, mutation-verified against three reverts -- and the third revert passed on the first attempt, so the fixture was deepened until it failed, which is exactly the check this stage exists to force. #97's four fixes are each covered by the test that was failing, verified before and after, with the OOM one additionally mutation-verified against the wrapper. #98 is covered by the pre-existing `test_qa_version_consistency.py`, which is a detector rather than a preventer and is recorded as such. #96 was subsequently fixed in `064189f` and its test is described in its own row -- worth reading, because the first version asserted on the layout's read-back values and passed against the defect, since d3 reports `tree.size()` as null whether nodeSize is the only sizing call or merely the last one; the assertion moved to the setter call sequence. #100 was found by §17's first check and is covered by that check. #103 is covered by the two version tests it was breaking, checked green on the node after the fix and red before. #101 and #102 are covered by the 19 tests that were failing, all of which now reach their own assertions; #101's entry states why its pass/fail *count* is not evidence and what is. Both were found only by sending the LOCAL_ONLY set to a QA node, which is the argument for doing that while the §0 gate keeps the local browser pass unavailable -- and the node turning out to have Chromium and Playwright browsers already installed means §14's "no Chromium is installed there" note is stale. #99 stays open and is the one entry with no test: the fixes are written and verified behaviour-neutral, but the file cannot be committed without shipping the broken feature they sit inside, and the measurements in that row say why. Nothing skipped.
...
# WAIT for the boot grace before this one. Case 1 restarted the server, and
# `boot_grace()` in wc-health.sh skips the whole check while MainPID is
# younger than WC_HEALTH_BOOT_GRACE (45s) -- deliberately, since a process
# too young to have served a request cannot be wedged. Run case 3 straight
# after case 1 and the health check exits silently, nothing is restarted,
# and the case reports a failure that is the sequencing, not the product.
# Measured 2026-09-17: it did exactly that at ~20s of uptime, printing
# nothing at all, and passed cleanly at 50s. The silence is what makes it
# read as a product failure rather than a skip.
while [ "$(python3 -c "import sysstats;print(int(sysstats.process_uptime_s($(systemctl --user show webconsole.service -p MainPID --value)) or 0))")" -lt 50 ]; do sleep 5; done
