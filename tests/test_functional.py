...
class AppPersistenceTests(unittest.IsolatedAsyncioTestCase):
    async def test_blocking_handler_persists_session(self):
        """Every database call the handler makes has to be mocked here.

        This drives the real handler with no database behind it, so the patch
        stack below is not a convenience -- it is the whole fixture. When
        `bump_chat_updated_at` was added to the handler this test began failing
        with `'NoneType' object has no attribute 'execute'`, because db_conn is
        None until init() runs and nothing had mocked the new call. It went
        unnoticed for a release: the file sat at the repository root, outside
        the glob the chunked runner used, so its failure was never in a total.
        """
