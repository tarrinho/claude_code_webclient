"""QA: the orchestrator fixes that landed in this session.
...
def _request(body=None):
    return SimpleNamespace(
        method="GET",
        url=SimpleNamespace(path="/api/orchestrator"),
        cookies={},
        headers={"accept": "*/*"},
        query_params={},
...
class CliDeduplicationTests(unittest.IsolatedAsyncioTestCase):
    ...
    section of the orchestrator output — the web path covers them.
    """
    ...
    async def test_web_linked_cli_session_is_skipped_in_cli_path(self):
        ...
        appear as a second entry in the orchestrator.
        """
        await _chat_with(
            "c1",
...
# ── Integration: full orchestrator output with cross-ref ───────────────────────

class FullSupervisorIntegrationTests(unittest.IsolatedAsyncioTestCase):
    """End-to-end orchestrator output with web chats linked to CLI sessions."""
