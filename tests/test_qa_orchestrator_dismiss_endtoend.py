...
So these drive the two real handlers in sequence:
    ...
    GET  /api/orchestrator        -> the row is listed as waiting
    POST /api/orchestrator/read   -> exactly what the dismiss control sends
    GET  /api/orchestrator        -> the row is no longer waiting
...
def _request(body=None, method="GET", path="/api/orchestrator"):
...
class DismissEndToEndTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _dismiss(self, kind="chat", ref_id=CHAT_ID):
        ...
        return json.loads((await supervisor_routes.handle_orchestrator_read(
            _request({"kind": kind, "id": ref_id, "dismiss": True},
                     method="POST", path="/api/orchestrator/read")
        )).body)
...
class DismissRequestValidationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _post(self, body):
        ...
        try:
            response = await supervisor_routes.handle_orchestrator_read(
                _request(body, method="POST", path="/api/orchestrator/read"))
