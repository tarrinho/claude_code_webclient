...
``web/orchestrator.html`` shipped a stray ``</div>`` that closed ``#layout``
...
def supervisor_source() -> str:
    ...
    """
    paths = sorted((REPO / "web" / "assets" / "orchestrator").glob("*.js"))
    if not paths:
        raise AssertionError(
            "no orchestrator modules found -- the split moved them somewhere "
...
def page_css() -> str:
    ...
    `orchestrator.html`'s inlined stylesheet block, so a test that greps this file
...
window.addEventListener("load", function () {
  ...
  document.title = JSON.stringify(out);
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # the relative `orchestrator.js` tag does -- and every geometry
