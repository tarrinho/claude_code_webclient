...
A orchestrator could only run work it invented itself: it decomposes a plan into
...
orchestrator to a conversation or to a running agent.
...
* **Ownership.** An unchecked ``ref_id`` would pull another account's
  conversation into a orchestrator the caller owns, exposing its title, preview
...
class StatusReuseTests(MembersBase):
    ...
    async def _sidebar_status(self, chat_id):
        """What /api/orchestrator says about this chat, for comparison."""
    ...
    async def test_a_failure_sorts_above_another_waiting_member(self):
        """A orchestrator is opened to find what needs attention.
