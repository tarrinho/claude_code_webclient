"""QA coverage for the bulk "+ Add members" picker on the orchestrator page.

The row menu adds one conversation as you come across it. This is the other
entry point: setting a orchestrator up in one pass, choosing several agents and
...
SUPERVISOR_MODULES = REPO / "web" / "assets" / "orchestrator"
...
def run_page(html: str, budget_ms: int = 20000) -> str:
    ...
    dumps in 0.6s, `web/orchestrator.html` alone dumps in 0.6s, and the same
    markup plus `web/orchestrator.js` never exits at all -- rc=124 under
...
def supervisor_source() -> str:
    """Every line of the orchestrator page's script, wherever it now lives.

    `web/orchestrator.js` is being split into ES modules under
    `web/assets/orchestrator/`. Reading only the legacy path would not fail when
    ...
    # to web/assets/orchestrator/main.js -- which is INSIDE SUPERVISOR_MODULES,
    ...
    if not parts:
        raise AssertionError(
            f"no orchestrator script found at {SUPERVISOR_JS} or "
...
def members_section() -> str:
    ...
    and resolves for 4xx. orchestrator.js's returns the PARSED BODY and throws on
    ...
    if SECTION_START not in source:
        ...
        raise AssertionError(
            ...
            "orchestrator script -- the section was renamed or the banners were "
...
class MarkupTests(unittest.TestCase):
    ...
    def setUp(self):
        if not SUPERVISOR_HTML.exists():
            self.skipTest("orchestrator.html not present")
...
class PickerTests(unittest.TestCase):
    ...
    def test_a_rejected_add_is_reported_as_an_error(self):
        result = self._run(add_ok=False, add_response={"error": "Orchestrator not found"})
        self.assertTrue(str(result.get("toast", "")).startswith("error:"),
                        f"a 400 must surface as an error, got {result.get('toast')!r}")
        self.assertIn("Orchestrator not found", str(result.get("toast")))
