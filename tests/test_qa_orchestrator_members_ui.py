"""QA coverage for the two ways a conversation joins a orchestrator.

The API can be right while the UI never reaches it. That is not hypothetical
here: the orchestrator page shipped with a getCsrf/apiFetch mutual recursion that
...
* An empty orchestrator list must say so. Rendering an empty box reads as a
...
SUPERVISOR_JS = REPO / "web" / "assets" / "orchestrator.js"
...
class MenuEntrySourceTests(unittest.TestCase):
    ...
    def test_the_menu_offers_the_action(self):
        self.assertIn("'Add to orchestrator', 'add-to-orchestrator'", self.source)

    def test_the_action_is_dispatched(self):
        """A button with no dispatch case is a menu entry that does nothing."""
        self.assertIn("action === 'add-to-orchestrator'", self.source)
    ...
    def test_the_label_is_a_bare_verb_like_its_neighbours(self):
        ...
        self.assertNotIn("'Add to orchestrator ' +", self.source)
        self.assertIn("makeButton('Add to orchestrator'", self.source)


class PickerSourceTests(unittest.TestCase):

    def setUp(self):
        # _addChatToSupervisor moved to orchestrator.js in a later module split;
...
class PickerBehaviourTests(unittest.TestCase):
    ...
    def _harness(self, supervisors, add_status=200, add_body=None):
        """Lift the picker out of orchestrator.js and run it against a stubbed
        fetch.
        ...
        * `export ` prefixes: legal on a real <script type="module">, a
          ...
          slice into.
        * `state.previousFocus`, not a bare `previousFocus`: orchestrator.js
    ...
    def test_a_rejected_add_is_reported_as_an_error(self):
        title = run_page(self._harness(
            ...
            add_status=404, add_body={"error": "Orchestrator not found"}))
        self.assertIn("error:", title)
        self.assertIn("Orchestrator not found", title)
