"""QA coverage for the orchestrator page reopening what you had open.

Pedro sent requests to a orchestrator and could not see them on the page. The
requests had arrived -- six user messages in the database -- and the page simply
never asked for them.

Nothing selected a orchestrator on load. ``selectSupervisor`` was reachable only
from a click on a list row, or immediately after creating one, so opening
/orchestrator.html always showed the start screen. And ``showActiveSupervisor``
...
indistinguishable from a orchestrator that had never been asked anything.
...
# `web/orchestrator.js` is being split into ES modules under this directory.
...
def lifted() -> str:
    ...
    if "function restoreOpen()" not in source:
        ...
        raise AssertionError(
            "restoreOpen() is not in the orchestrator script. It is implemented, "
...
class SourceWiringTests(unittest.TestCase):
    ...
    def test_the_script_tag_was_cache_busted(self):
        ...
        # Matches the module path rather than the old `orchestrator.js`: the
        # script moved to web/assets/orchestrator/ in 0.10.0 so StaticFiles could
        ...
        self.assertIsNotNone(
            match, "the orchestrator module must carry a version query")
...
class RestoreBehaviourTests(unittest.TestCase):
    ...
    def test_an_empty_list_selects_nothing(self):
        """A first run has no orchestrator; selecting one must not be invented."""
