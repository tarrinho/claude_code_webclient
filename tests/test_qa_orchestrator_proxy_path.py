"""QA coverage for the orchestrator reaching its backend through the proxy.

The owner fallback that let the orchestrator authenticate at all was added and
...
So in the only configuration that matters the orchestrator was still
...
class ProxyTurnFrameTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _send(self, owner):
        """One orchestrator-shaped turn: an id that is not a conversation."""
...
class BothBranchesAreCoveredTests(unittest.TestCase):
    ...
    def test_no_resolution_site_ignores_the_owner(self):
        ...
        for pattern in ("get_backend(chat_id)", "get_default_model(chat_id)"):
            self.assertNotIn(
                pattern, source,
                f"{pattern} resolves without an owner; the orchestrator's ids are "
