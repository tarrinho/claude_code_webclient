"""QA: the orchestrator page is reachable at the path people type."""
...
class SupervisorRouteQA(unittest.IsolatedAsyncioTestCase):
    ...
    "/orchestrator" returned 404 while "/orchestrator.html" worked. Nothing in the
    ...
    def test_both_paths_are_registered(self):
        ...
        self.assertIn("/orchestrator.html", registered)
    ...
    def test_the_long_form_is_kept_because_the_app_asks_for_it(self):
        ...
        # openSupervisorPane's navigation moved from app.js to orchestrator.js
        # in a later module split.
        supervisor_js = (WEB / "assets" / "orchestrator.js").read_text(encoding="utf-8")
        self.assertIn("orchestrator.html", supervisor_js)
    ...
    async def test_both_serve_the_same_page(self):
        ...
        # moved to /assets/orchestrator/main.js in 0.10.0; the assertion is still
        # that the page names its script and busts the cache.
        self.assertIn("/assets/orchestrator/main.js?v=", body)
    ...
    def test_every_asset_the_page_requests_has_a_route(self):
        ...
        """
        import re
        html = (WEB / "orchestrator.html").read_text(encoding="utf-8")
        registered = self._routes()
