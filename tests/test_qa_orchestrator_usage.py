"""QA: a orchestrator turn's cost is recorded, and attributed to the orchestrator.
...
`orchestrator.py` never did — so a orchestrator fanning out ten subtasks spent ten
...
`origin="orchestrator"` rather than the default `"web"`: the column exists so spend
...
class UsageRecordingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_it_is_attributed_to_the_supervisor(self):
        ...
        rows = await self._record(self._frame())
        self.assertEqual(rows[0]["origin"], "orchestrator")
...
class CallSiteTests(unittest.TestCase):
    ...
    it looks exactly like a orchestrator that happened to be cheap."""
    ...
    def test_the_task_records_on_both_the_success_and_failure_paths(self):
        ...
        rather than "both paths record". A orchestrator whose successful turns cost
