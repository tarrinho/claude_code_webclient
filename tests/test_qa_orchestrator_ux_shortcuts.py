"""QA: the five orchestrator UX affordances, driven in a real browser.
...
``orchestrator.js`` is an IIFE, so nothing inside is reachable by name -- which is
...
HTML alone exits in 1s, the HTML plus ``orchestrator.js`` hangs, and the two files
...
# Replaces EventSource before any page script runs. orchestrator.js publishes the
...
class _SupervisorPage(unittest.TestCase):
    ...
    def _install_routes(self):
        ...
        def handler(route):
            ...
            # Serve any file under /assets/orchestrator/ from disk, by name,
            # rather than one hard-coded script.
            #
            # The script is an ES module now, so it fetches its own imports:
            # matching only "orchestrator/main.js" fulfilled the entry point and
            ...
            # selector -- every test timed out waiting for `.orchestrator-list-item`
    ...
    def open_page(self, *, select=True):
        """Load the orchestrator page, optionally selecting the one orchestrator.
        ...
        """
        self.page.goto(f"{ORIGIN}/orchestrator.html", wait_until="domcontentloaded")
        self.page.wait_for_selector(".orchestrator-list-item", timeout=10_000)
        if select:
            self.page.click(".orchestrator-list-item .sl-title")
    ...
    def push(self, frame: dict):
        ...
        self.page.evaluate(
            """(frame) => {
                ...
                if (!es) throw new Error('no SSE stream: orchestrator not selected');
...
class GoalBannerSlimStripTests(_SupervisorPage):
    ...
    def test_the_restore_survives_the_next_message(self):
        ...
        nothing, because on a live orchestrator the next message is a second away.
        """
        self.open_page()
        self.send_prompt("Refactor the parser")
        self.push_message("working on it")
