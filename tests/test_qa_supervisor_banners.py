"""QA coverage for the supervisor goal banner and completion banner.
...
class GoalBannerPlacementTests(unittest.TestCase):
    ...
    def test_goal_banner_inside_supervisor_chat(self):
        """The banner must be inside the #supervisor-chat container, not floating."""
        chat_open = self.html.find('id="supervisor-chat"')
        goal_close = self.html.find('class="goal-banner"', chat_open)
        chat_close = self.html.find("</div>", goal_close)
        self.assertNotEqual(chat_close, -1, "supervisor-chat div must close after goal-banner")
    ...
    def test_completion_banner_inside_supervisor_chat(self):
        ...
        self.assertNotEqual(chat_close, -1, "supervisor-chat div must close after completion-banner")
...
class BrowserIntegrationTests(unittest.TestCase):
    """End-to-end tests that run the supervisor page in a real browser."""
