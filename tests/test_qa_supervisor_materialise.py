...
new method, because `supervisor.py` imports `db` inside functions (a
module-level import is a cycle: `db` imports `supervisor` at load time) and the
...
* Every supervisor's `PlanParser` numbers tasks from 1, so every plan produces a
  `t001`, and `supervisor_tasks.id` is a global primary key. The second
  supervisor's writes failed on the UNIQUE constraint, were swallowed by a bare
...
class MaterialisePlanTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.engine = supervisor.SupervisorEngine(self.sup, "alice")

    @staticmethod
    def _tasks(*specs):
        return [
            supervisor.ParsedTask(
    ...
    async def test_row_ids_are_namespaced_so_two_supervisors_can_both_have_t001(self):
        ...
        PlanParser numbers from 1 within a plan, so every supervisor emits a
        ...
        engine2 = supervisor.SupervisorEngine(other, "alice")
        ...
        self.assertEqual(len(mine), 1, "the first supervisor's task is missing")
        self.assertEqual(len(theirs), 1,
                         "the second supervisor's task was lost to an id collision")
