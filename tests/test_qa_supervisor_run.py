"""QA coverage for a supervisor actually running a task.
...
7. **Task rows collided across supervisors.** ``PlanParser`` numbers from 1 per
   ...
   supervisor's insert failed on the UNIQUE constraint -- swallowed by a bare
   warning, so the task ran to completion while the list stayed empty.
8. **The supervisor's progress was never written.** Task rows carried theirs;
   the supervisor row did not, so every finished run still displayed 0%.
...
class PlanParsingTests(unittest.TestCase):
    ...
    def test_the_title_is_the_title_not_the_task_number(self):
        ...
        tasks = supervisor.PlanParser.parse(
    ...
    def test_a_line_with_no_dash_is_all_title(self):
        ...
        tasks = supervisor.PlanParser.parse("<<PLAN\nTask 1: Just do the thing\n>>")
        self.assertEqual(len(tasks), 1)
    ...
    def test_a_markdown_table_yields_no_tasks(self):
        ...
        self.assertEqual(supervisor.PlanParser.parse(table), [])
    ...
    def test_the_prompt_and_the_parser_agree_on_the_delimiter(self):
        ...
        for shown in set(re.findall(r"<<PLAN>?>?", text)):
            self.assertTrue(
                supervisor._PLAN_START_RE.search(shown.rstrip(".") + "\n")
                or shown in ("<<PLAN>>..>>",),
...
class TaskRowIdentityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_the_schema_makes_a_plan_local_id_collide(self):
        ...
        PlanParser numbers from 1 within a plan, so every supervisor produces a
        t001. Writing that raw meant the second supervisor's insert failed on
    ...
    async def test_a_namespaced_id_lets_two_supervisors_both_have_task_one(self):
        ...
        for sup in sups:
            ...
            self.assertEqual(len(rows), 1, "each supervisor keeps its own task")
