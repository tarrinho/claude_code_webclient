"""QA: the supervisor scheduler must be able to finish, and to say it failed.
...
half-second intervals for ever with nothing runnable, the supervisor never
...
def graph_with(**statuses: str) -> supervisor.TaskGraph:
    graph = supervisor.TaskGraph()
    for task_id, status in statuses.items():
        graph.add_task(
            supervisor.TaskNode(id=task_id, title=task_id, description="", model=None)
        )
...
class TerminalStateTests(unittest.TestCase):
    ...
    def test_an_empty_graph_is_finished(self):
        self.assertTrue(supervisor.TaskGraph().all_done())
...
class ScheduleLoopTests(unittest.IsolatedAsyncioTestCase):
    ...
    The engine updated a graph node called "supervisor" that nothing ever
    ...
    def _engine(self, graph: supervisor.TaskGraph) -> supervisor.SupervisorEngine:
        engine = supervisor.SupervisorEngine("sup-1", "admin")
        engine.graph = graph
        ...
        patcher = patch.object(
            supervisor.SupervisorEngine, "_set_status",
    ...
    async def test_an_exception_inside_the_loop_is_logged_and_ends_it(self):
        ...
        with patch.object(engine, "_execute_task",
                ...
                self.assertLogs("wc.supervisor", level="ERROR") as logs:
            await asyncio.wait_for(engine.run_schedule_loop(), timeout=5)
        self.assertFalse(engine._running)
        self.assertEqual(self.persisted[-1], "error")
...
class BackgroundTaskTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_spawn_keeps_a_reference_while_it_runs(self):
        engine = supervisor.SupervisorEngine("sup-2", "admin")
    ...
    async def test_the_reference_is_released_when_it_finishes(self):
        ...
        engine = supervisor.SupervisorEngine("sup-3", "admin")
    ...
    async def test_a_failure_in_a_spawned_task_is_logged(self):
        ...
        engine = supervisor.SupervisorEngine("sup-4", "admin")

        async def boom():
            raise RuntimeError("planner exploded")

        with self.assertLogs("wc.supervisor", level="ERROR") as logs:
    ...
    async def test_cancelling_is_not_reported_as_a_failure(self):
        engine = supervisor.SupervisorEngine("sup-5", "admin")
