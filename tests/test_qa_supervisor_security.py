"""QA: findings from the 0.9.3 threat-modelling pass on the supervisor surface.

`docs/threat-model.md` analysed 0.7.2 and says so, listing the supervisor
...
read any supervisor's task titles, descriptions and results -- agent output --
...
class SupervisorTenancyTests(unittest.IsolatedAsyncioTestCase):
    """One account must not read another's supervisor by id."""
    ...
    async def asyncSetUp(self):
        ...
        await db.supervisor_messages_append(
            "sup-bob", "supervisor", "BOB-PRIVATE-MESSAGE", {"kind": "plan"}
        )
        await db.supervisor_create("sup-alice", "Alice's", "", "alice")
        for n in range(3):
            await db.supervisor_messages_append(
                "sup-alice", "supervisor", f"ALICE-{n}", {}
            )
