...
class ProviderClassificationQA(unittest.TestCase):
    ...
    def test_official_api_is_trusted_for_cost(self):
        for machine in (
            ...
            self.assertEqual(app._usage_provider(machine), "anthropic", repr(machine))
    ...
    def test_a_gateway_is_not_trusted_for_cost(self):
        ...
        for base in (GATEWAY, "http://10.0.0.5:4000/llm", "https://litellm.internal"):
            self.assertEqual(
                app._usage_provider({"provider": "anthropic", "base_url": base}),
                "anthropic-compatible",
                base,
            )

    def test_proxy_and_missing_machines_are_untrusted(self):
        self.assertEqual(app._usage_provider({"provider": "proxy"}), "proxy")
        self.assertEqual(app._usage_provider(None), "proxy")
        self.assertEqual(app._usage_provider({}), "proxy")
