...
async function _loadMap(quiet = false) {
  ...
  try {
    ...
    const { renderSupervisorMap, closeSupervisorMap: closeMap } = await import('./supervisor-map.js?v=9204602');
    if (closeMap) closeMap();
    renderSupervisorMap(data);
    byId('mapStatusEmpty').textContent = MAP_EMPTY_TEXT;
  } catch {
    if (quiet) return;
    const { closeSupervisorMap } = await import('./supervisor-map.js?v=9204602');
...
async function _switchTab(tab) {
  ...
  if (tab === 'delegation') {
    const {loadDelegation} = await import('./delegation.js?v=13897877');
    loadDelegation(true);
  }
}
...
import { startServerPolling, stopServerPolling, loadServer, notifyResult, setStatus } from './server-stats.js?v=1683277';
...
import { loadMachines, _activateMachine, _editMachine, _saveMachine, _showAddMachine, _syncMachineProviderFields, _modelsByMachine, _renderMachineList,
  ...
  _drawMapWires, _pollTunnelStatus, _collapseAllTransportGroups, _closeConfirmDialog } from './machines.js?v=6755710';

import { loadTransports, _transports, _showAddTransport, _cancelTransportForm, _testTransportForm, _saveTransport } from './transports.js?v=8464275';
